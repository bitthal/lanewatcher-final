  ...
  const fetchData = async (date) => {
    ...
    try {
      ...
      await axios
        ...
        .then((response) => {
          if (response.data.lanewise_data.length > 0) {
            setLoader(false);
            setNodataPage(false);
    ...
    <>
      <div className="flex flex-col gap-5 my-3 mr-3 h-auto ">
        ...
        {loader ? (
            ...
            <table className="w-full border-collapse border border-[#cccc]">
              <tbody>
                ...
                <tr className="border border-[#cccc]">
                  <td className="border border-[#cccc] p-2 text-center text-[#434190]">
                    Total Monotainers
                  </td>
                  <td className="border border-[#cccc] p-2 text-center text-[#434190] text-xl">
                    0
                  </td>
                </tr>
          ...
          <div className="flex flex-boxShadow flex-col md:flex-row justify-center mt-10 p-5 w-50 m-auto">
           <p className="text-center lg:text-xl text-base"> No data availabe !!</p>
          </div>
        ) : (
          <div
                ...
                filteredData?.map((data, index) => (
                  <div className="flex m-auto" key={index}>
                    ...
                    <PlanogramView
                      ...
                      cameraId={totalStats[0].camera_id}
                    />
                    {/* </div> */}
          ...
          <div className="right-card w-25 m-2 pl-1 xl:h-[80vh] h-[70vh]">
            ...
            <table className="w-full border-collapse border border-[#cccc]">
              ...
              {totalStats.map((data, index) => (
                <tbody key={index}>
                  <tr className="border border-[#cccc]">
                    <td className="border border-[#cccc] p-2 text-center text-[#434190] lg:text-xl text-md">
                      Total Monotainers
                    </td>
                    <td className="border border-[#cccc] p-2 text-center text-[#434190] text-xl lg:text-xxxl">
                      {data?.Total_monoid ? data?.Total_monoid : 0}/
                      {data?.Total_master_monoids
                        ? data?.Total_master_monoids
                        : 0}
                    </td>
                  </tr>
